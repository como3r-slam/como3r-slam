import torch
from como3r_slam.frame import Frame
...
from como3r_slam.nonlinear_optimizer import check_convergence, huber
from como3r_slam.config import config
from como3r_slam.mast3r_utils import mast3r_match_asymmetric
...
class FrameTracker:
    ...
    def solve(self, sqrt_info, r, J):
        ...
        # Marquardt damping: H_lm = H + lambda * diag(H) + eps * I.
        # Pure GN is sensitive to rank-deficient H (most pixels masked out,
        # near-degenerate geometry); damping makes the system always SPD so
        # Cholesky succeeds and the step gracefully degrades toward gradient
        # descent. Retry with progressively larger lambda if Cholesky still
        # fails (extremely ill-conditioned H).
        diag_H = torch.diagonal(H)
        I = torch.eye(mdim, device=H.device, dtype=H.dtype)
        eps_floor = self.cfg.get("lm_eps_floor", 1e-9)
        lam = self.cfg.get("lm_lambda_init", 1e-6)
        lam_max = self.cfg.get("lm_lambda_max", 1e3)
        tau_j = None
        while True:
            H_lm = H + lam * torch.diag(diag_H) + eps_floor * I
            try:
                L = torch.linalg.cholesky(H_lm, upper=False)
